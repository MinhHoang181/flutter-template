    ...
    if (containUppercase && !value.contains(_uppercaseRegex)) {
      return {
        noUppercase: App.text(
          'core.validate.password.no_uppercase',
    ...
    if (containLowercase && !value.contains(_lowercaseRegex)) {
      return {
        noLowercase: App.text(
          'core.validate.password.no_lowercase',
    ...
    if (containSpecialChar && !value.contains(_specialCharRegex)) {
      return {
        noSpecialChar: App.text(
          'core.validate.password.no_special_char',
...
class AppDateValidator extends Validator<DateTime?> {
  ...
  Map<String, dynamic>? validate(AbstractControl<DateTime?> control) {
    ...
    final minVal = min;
    if (minVal != null && value.isBefore(minVal)) {
      final String minText = minVal.showDate();
      return {
        lessThanMin: App.text(
    ...
    final maxVal = max;
    if (maxVal != null && value.isAfter(maxVal)) {
      final String maxText = maxVal.showDate();
      return {
        greaterThanMax: App.text(
