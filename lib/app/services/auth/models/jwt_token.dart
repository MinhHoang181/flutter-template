...
/// A JSON Web Token (JWT) token.
class JwtToken {
  /// Creates a new JWT token with the given [value].
  const JwtToken(this.value);

  /// The value of the token.
  final String value;

  /// The date and time when the token was expired.
  ///
  /// If the token does not have an expiration date, this should return `null`.
  DateTime? get expiration {
    try {
      return JwtDecoder.getExpirationDate(value);
    } catch (_) {
      return null;
    }
  }
  ...
  /// The value of the token to be used in the `Authorization` header.
  String get authorizationValue => 'Bearer $value';

  /// Returns the payload of the token.
  Map<String, Object?>? get payload => JwtDecoder.tryDecode(value);
}
...
class JwtDecoder {
  ...
  static DateTime? _getDate({required String token, required String claim}) {
    ...
    final expiration = int.tryParse(decodedToken[claim].toString());
    if (expiration == null) {
      return null;
    }
